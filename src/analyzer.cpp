...
Analyzer::Analyzer(const json& config) {
    thresholds_ = config.value("thresholds", json::object());
    if (thresholds_.empty()) {
        std::cout << "Analyzer: thresholds bulunamadi (config eksik)\n";
    } else {
        std::cout << "Analyzer: " << thresholds_.size()
                  << " esik tanimi yuklendi\n";
    }
}

const json* Analyzer::get_threshold(const std::string& sensor,
                                     const std::string& metric) const {
    std::string key = sensor + "." + metric;
    if (thresholds_.contains(key)) return &thresholds_[key];
    return nullptr;
}
...
json Analyzer::analyze(const std::string& sensor,
    ...
    if (!th) {
        return json::object();  // esik yok -> analiz yok
    }
    ...
    std::string status;
    if (value < tmin)      status = "dusuk";
    else if (value > tmax) status = "yuksek";
    else                   status = "normal";
...
json Analyzer::analyze_stats(const std::string& sensor,
    ...
    // Esik disina cikan deger var mi?
    ...
    std::string reason;
    if (any_anomaly) {
        // Magnitude: ne kadar aciktan asti
        double margin = 0;
        ...
        if (max_out && min_out)
            reason = "min " + std::to_string(min_v) + " ve max " +
                     std::to_string(max_v) + " esik disinda";
        else if (max_out)
            reason = "max " + std::to_string(max_v) +
                     " > esik " + std::to_string(tmax);
        else
            reason = "min " + std::to_string(min_v) +
                     " < esik " + std::to_string(tmin);
    }

    // Anlik (last) durumu
    std::string last_status;
    if (last_v < tmin)      last_status = "dusuk";
    else if (last_v > tmax) last_status = "yuksek";
